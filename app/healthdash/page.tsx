...
import { subscribeToHealthData } from "@/lib/firebase-service";
import type { HealthData } from "@/lib/types";
import { subscribeToDiagnosticResults } from "@/lib/firebase-service";
...
export default function Dashboard() {
  ...
  const [patientHistory, setPatientHistory] = useState<HealthData[]>([]);
  ...
  // Subscribe to patient health records (last 30 days)
  useEffect(() => {
    setHistoryLoading(true);
    setHistoryError(null);
    try {
      const unsubscribe = subscribeToHealthData(30, (data) => {
  ...
  // Subscribe to diagnostic results (last 30 days)
  useEffect(() => {
    setDiagLoading(true);
    try {
      const unsub = subscribeToDiagnosticResults(30, (data) => {
        setDiagnostics(data || []);
                      ...
                      <div className="col-span-2 mt-4">
                        <Card>
                          <CardHeader>
                            ...
                            <CardDescription>
                              Latest AI diagnostic results (30 days)
                            </CardDescription>
                          </CardHeader>
                          <CardContent>
                            {diagLoading ? (
                  ...
                  <CardContent>
                    {historyLoading ? (
                      ...
                      <div className="space-y-3">
                        {patientHistory.slice(0, 6).map((rec) => (
                          <div
                            key={rec.id}
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between">
                                <p className="font-medium">
                                  {rec.date || formatTimestamp(rec.timestamp)}
                                </p>
                                <Badge variant="outline" className="text-xs">
                              ...
                              <p className="text-sm text-muted-foreground mt-1">
                                Heart: {rec.heartRate ?? "—"} bpm · BP:{" "}
                                {rec.bloodPressure?.systolic ?? "—"}/
                                {rec.bloodPressure?.diastolic ?? "—"} mmHg ·
                                Steps: {rec.steps ?? "—"}
                              </p>
                              {rec.notes ? (
